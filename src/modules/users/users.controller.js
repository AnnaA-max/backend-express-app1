...
// :x:route handler: get all users (mock)
export const getUsers1 = (req, res) => {
  res.status(200).json(users);
  console.log(res);
};

// :white_check_mark: route handler: get all users from the database
...
export const deleteUser2 = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      erroe: "Failde to delete user..."
    })
  }
};

// :x:route handler: create a new user (mock)
...
// :heavy_check_mark: route handler: craete a new user in the database
export const createUser2 = async (req, res) => {
  ...
  try {
    const doc = await User.create({ username, email, password, role }); //จัดให้ตรงschema  ตรงแล้วจะcreate เอาสิ่งที่รีเทิน ส่งไปrespond
    const safe = doc.toObject();  // change to Obj the keep in safe
    delete safe.password; // dot in Obj and delete password filed(ถ้ามีpasswมาด้วย)
